...
from algorithms.SHBPPO.SHBPPO_agent import SHBPPO
from environment.my_env import MyEnv
from utils.normalization import Normalization, RewardScaling


def _load_shbppo_main_helpers():
    current_dir = os.path.dirname(os.path.abspath(__file__))
    shbppo_main_path = os.path.join(_OUTER_ROOT, "algorithms", "SHBPPO", "SHBPPO_main.py")
    spec = importlib.util.spec_from_file_location("shbppo_main_helpers", shbppo_main_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_shbppo_main_helpers = _load_shbppo_main_helpers()
setSeed = _shbppo_main_helpers.setSeed
get_madrl_args = _shbppo_main_helpers.get_madrl_args
get_base_args = _shbppo_main_helpers.get_base_args
split_and_discretize = _shbppo_main_helpers.split_and_discretize
...
if __name__ == "__main__":
    ...
    # 保持 UAV 高度默认值不变（uav_height=100），扫描雷达 RCS
    checkpoint_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint", "checkpoint_shbppo")
    actor_path = os.path.join(checkpoint_dir, "shbppo_actor.pth")
    critic_path = os.path.join(checkpoint_dir, "shbppo_critic.pth")

    radar_rcs_values = [2, 4, 6, 8, 10, 12]
    all_results = []

    for rcs in radar_rcs_values:
        setSeed(seed=test_seed)
        base_args.seed = test_seed
        base_args.radar_rcs = rcs
        ...
        # NOTE - 单头动作维度 = 连续动作维度 + uavs_num（每个 UAV 的离散 CU 选择当作一个 [0,1] 维）
        continuous_dim = sum(env.bs_continuous_action_splits.values())
        discrete_num = int(base_args.uavs_num)
        total_action_dim = continuous_dim + discrete_num
        ...
        for warm_idx in range(1):
            ...
            while not terminal_warm:
                ...
                raw_action_warm, _ = agent_bs.choose_action(state_bs_norm_warm)
                continuous_part_warm, cu_indices_warm = split_and_discretize(
                    raw_action_warm, continuous_dim, base_args.uavs_num, base_args.cus_num
                )
                env_action_warm = {"continuous": continuous_part_warm, "discrete": cu_indices_warm}
                next_s_warm, _, _, done_warm, _, _ = env.step({"bs": env_action_warm}, i_episode=test_seed)
                s_warm = next_s_warm
        ...
        while not terminal:
            raw_action_bs, _ = agent_bs.choose_action(state_bs_norm)

            # NOTE - 单头边界映射：把后 uavs_num 维 [0,1] 值离散化为 CU 索引，再组装成环境格式
            continuous_part, cu_indices = split_and_discretize(
                raw_action_bs, continuous_dim, base_args.uavs_num, base_args.cus_num
            )
            env_action_bs = {"continuous": continuous_part, "discrete": cu_indices}

            next_s, total_reward, r_dict, done, obj_fun, success_flag = env.step({"bs": env_action_bs}, i_episode=test_seed)
    ...
    filename = f"{current_time_str}_SHBPPO_test_radar_rcs_results.csv"
